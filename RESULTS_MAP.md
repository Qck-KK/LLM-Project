...
| `results_ablations/` | Paired comparisons against the final heads: PQM vs BCE loss (`loss_pqm_vs_bce.*`, `bon_bce/`), `zeta` 2 / 8 (`zeta.*`), seeds 43 / 44 (`seeds.*`), learning rates 3e-5 / 3e-4 (`lr.*`), and a 60-epoch cap for mlp and attention_pe (`long.*`). Training logs in `train/`. |
